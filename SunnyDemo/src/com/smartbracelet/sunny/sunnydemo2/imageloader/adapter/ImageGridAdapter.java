package com.smartbracelet.sunny.sunnydemo2.imageloader.adapter;
...
public class ImageGridAdapter extends BaseAdapter {
    ...
    private List<LocalImage> mImages = new ArrayList<>();
     ...
     * @param image
     */
    public void select(LocalImage image) {
    ...
    public void setDefaultSelected(ArrayList<String> resultList) {
        for(String path : resultList){
            LocalImage image = getImageByPath(path);
            if(image != null){
                mSelectedImages.add(image);
            }
        }
    ...
    private LocalImage getImageByPath(String path){
        if(mImages != null && mImages.size()>0){
            for(LocalImage image : mImages){
     ...
     * @param images
     */
    public void setData(List<LocalImage> images) {
    ...
    class ViewHolde {
        ...
        void bindData(final LocalImage data){
            if(data == null) return;
            // 处理单选和多选状态
